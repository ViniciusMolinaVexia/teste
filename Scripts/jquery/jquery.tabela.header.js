(function ($) {
    ...
    $.fn.tabelaHeader = function (opcoes) {
        var idHeader = '#' + $(this).attr('id') + 'H'; 
        var tabela = $(this);
        ...
        if (opcoes.acao === "carregar") {
            ...
            carregarHeader($(this), idHeader, function () {
        ...
        $(idHeader).find('a').livequery(function () {
            if ($(this).data('ordenacao') > 0)
                $(this).click(function () { aoOrdenarHeader($(this), tabela, idHeader); });
        });

        carregarHeader(tabela, idHeader, function () {
    ...
    function aoRolarAteOFim(conteudo) {
        ...
        if (conteudo.scrollTop() + conteudo.innerHeight() >= conteudo[0].scrollHeight) {
            ...
            parametros.pagina = Math.ceil((trLength / fatorLinhas - 1) / tamanhoPagina) + 1;
    ...
    function aoOrdenarHeader(botao, tabela, idHeader) {
    ...
    function aposOrdenarHeader(idHeader) {
        if (parametros.ordenacao && parametros.tipo) {
            var linkOrdenacao = $(idHeader).find('[data-ordenacao=' + parametros.ordenacao + ']');
            if (linkOrdenacao) {
    ...
    function carregarHeader(tabela, idHeader, callbackCarregar) {
